...
def scan_network(config: dict, target_range: str) -> dict[str, Any]:
    ...
    ports = discovery_cfg.get("ports", [22, 80, 443, 3306, 5432, 8006, 8080])
...
def audit_from_csv(config: dict, csv_path: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"total_hosts": 0, "reachable": 0, "unreachable": 0, "eol_hosts": 0, "hosts": [], "error": str(exc)}
    ...
    for row in rows:
        ...
        # Check connectivity via SSH (port 22) as a basic reachability test
        is_reachable = False
        if ip:
            is_reachable = check_port(ip, 22, timeout=timeout)
            if not is_reachable:
                # Fallback: try ICMP-like with port 80
...
def generate_report(config: dict) -> dict[str, Any]:
    ...
    Returns:
        Dict with keys: report_path, scan, inventory, eol.
    """
